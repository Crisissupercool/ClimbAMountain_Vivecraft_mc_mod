...
**Current state (after 2026-07-02 autonomous session):** the full non-nail stack exists and builds green — all THREE handhold blocks (slab outcrop + vertical seam on the shared `AbstractWallHoldBlock` base), the data-driven slip registry + `/climbslip` debug command, Vivecraft + Lithostitched wired as dependencies, the **Vivecraft climbing hookup** (grip via tag bridge + slip mixin, boot-verified, awaiting VR feel-test), and **worldgen** (cliff-detection feature + Lithostitched biome modifier, verified generating naturally on a dev server). Remaining big rock: the nail system. The **rock nub** was previously verified in-game (`runClient`): places on all four walls facing outward, 3×3 aim-based positioning, multi-nub clusters, collision + waterlogging confirmed. Compiles (common + client); datagen runs and emits the loot table AND the two block tags. **Block-tag scaffolding now exists** — `climbamountainvr:handhold` + `climbamountainvr:weak_handhold` are datagenned (see Tag scaffolding note below); these are the tags the future Vivecraft mixin will consult. Climbing/grip NOT wired yet (needs Vivecraft dep, separate task). Build/run is healthy on this machine.
...
**Slab outcrop + vertical seam (built, datagen-clean, NOT yet verified in-game):**
- **Shared base `block/AbstractWallHoldBlock.java`** — extracted from RockNubBlock, now the parent of all three holds. Owns: `WATERLOGGED` + fluid plumbing, `rotate`/`mirror` (FACING only), `facingIndex`, `rotateBoxY` (NORTH-authored boxes → other walls by the blockstate's y-rotation, so collision/visuals can't drift), `slot(fraction)` and `aimedSlot(ctx, facing)` (the chirality-safe 3×3 aim math). RockNubBlock was refactored to extend it — pure code motion, no behavior change.
- **`block/SlabOutcropBlock.java`** (the jug): ONE ledge per cell — 14px wide × 3 thick × 5 deep, `LEVEL` IntProperty 0–2 (low/mid/high, from vertical aim). LEVEL=2's top is flush at y=16 so it doubles as a step aligned with the block above's floor. No merge/cluster logic (a ledge already spans the cell width).
- **`block/VerticalSeamBlock.java`** (the rail): ONE thin ridge per cell — 4px wide × FULL 16px tall × 3 deep, `COLUMN` IntProperty 0–2 (left/centre/right on the same 4/8/12px grid centres as the nub, from horizontal aim). The "~2 blocks tall" seam from the taxonomy = STACK two; full-height geometry welds vertically-adjacent seams into one continuous rail (same emergence trick as nub→slab). Avoids door/tall-plant two-half machinery entirely; worldgen gets 1/2/3-tall seams free.
- **Blockstates are `variants` (not multipart)** for both — exactly one (facing, level/column) applies, so 3 models + 12-variant map each. Scripts: `scripts/gen_slab_outcrop_models.py`, `scripts/gen_vertical_seam_models.py` (same keep-dims-EQUAL-to-makeShapes contract as the nub script). Placeholder textures via `scripts/gen_placeholder_textures.py` (stdlib PNG writer, no PIL on this machine): outcrop tinted lighter, seam darker, so hold types read apart in-game.
- Registered with items + creative tab (`ItemGroups.NATURAL`), loot tables datagenned. Not craftable, as intended.

**Slip registry (built, datagen/build-clean, NOT yet verified in-game):**
- **`slip/SlipRegistry.java` (common)** — static lookup: block id → slip chance/tick while gripped. Default 0.0 for unlisted blocks. The map reference is swapped atomically (volatile + immutable snapshot) on each datapack reload — lock-free reads on the per-tick hot path. This is the shared infrastructure both the climbing mixin AND the future nail wall-slip will consult.
- **`slip/SlipDataLoader.java` (common)** — implements vanilla `SynchronousResourceReloader`, registered via **fabric-resource-loader-v1** `ResourceLoader.get(ResourceType.SERVER_DATA).registerReloader(id, loader)` in `onInitialize`. GOTCHA: the classic v0 `SimpleSynchronousResourceReloadListener`/`ResourceManagerHelper` path is DEPRECATED in Fabric API 0.141.4 — v1 takes a plain vanilla `ResourceReloader` + an Identifier instead of the `getFabricId()` interface. Merges every `data/<ns>/climb_slip/*.json` (flat object: `"block_id": slipPerTick`) across namespaces in sorted-file-id order; standard pack stacking overrides within one file id.
- **Seed data:** `src/main/resources/data/climbamountainvr/climb_slip/default.json` — slab_outcrop 0.002, vertical_seam 0.01, rock_nub 0.02, vine 0.0 (explicit-zero example). Placeholder values, tune in VR.
...
**Vivecraft climbing hookup (built + boot-verified; NEEDS VR FEEL-TEST):**
- **Grip = pure data, no mixin.** `ModBlockTags.VIVECRAFT_CLIMBABLE` (`vivecraft:climbable`) is datagenned to contain `#climbamountainvr:handhold` + `#climbamountainvr:weak_handhold` → merges with Vivecraft's own tag file at runtime → every handhold passes `ClimbTracker.isClimbableBlock`. This is deliberately the CLAUDE.md architecture goal (extend the block-level decision, not an equipment gate) achieved more robustly than the planned mixin — mixin fragility only where unavoidable.
- **Slip = one mixin.** `client/mixin/ClimbTrackerMixin` (client sources, registered in `climbamountainvr.client.mixins.json`) injects at TAIL of `ClimbTracker.activeProcess`: while latched, looks up the gripped block's slip (floors `latchStart`, and ALSO probes one cell along `grabDirection` since the 0.2-thick grab slab straddles the wall plane — takes the max), rolls per tick, and on failure clears `latched[0]/[1]` (exactly what Vivecraft's own button-release does; its next tick restores gravity) + fires a strong haptic pulse (`triggerHapticPulse(ControllerType, int)` — the int,int overload is deprecated). Uses `@Shadow(remap = false)` on Vivecraft fields (mod classes aren't obfuscated; only MC refs remap).
- **Mixin application is boot-verified via a tripwire:** `ClimbAMountainVrClient.onInitializeClient` force-loads `ClimbTracker.class` and logs it. Vivecraft loads that class lazily (only entering VR), so without this a broken mixin passes flatscreen boots and explodes in the headset; now it fails at launch. Log line to look for: `Vivecraft climb hook target loaded: ...ClimbTracker`.
- `fabric.mod.json` now `depends` on `vivecraft` (mixin into its class = hard dep).
- **WHAT TO TEST IN VR:** (1) rock nub / slab outcrop / vertical seam should be grippable bare-handed with "Realistic Climbing" enabled in Vivecraft settings (no claws needed); leaves/vines too (were already vanilla-climbable? vines yes — the new ones are our tag additions like leaves). (2) Grip a rock nub and hang: with slip 0.02/tick it should pop your grip after ~2.5 s on average, with a haptic thump — slab outcrop should hold ~25 s, seam ~5 s. (3) After a slip, re-gripping requires releasing + re-pressing grab. (4) Grab-side check: holds should only grab from their FACING side (the wall plane they protrude from).

**Worldgen scaffolding (built + VERIFIED generating on a dev server; tuning deferred):**
- **Code:** `worldgen/CliffHandholdsFeature.java` (common) — `Feature<DefaultFeatureConfig>`. Column-scan cliff detection per the spec: from the heightmap origin walk down ≤64 blocks, a solid block with ≥1 horizontal air-or-water neighbour is "exposed"; a run of ≥5 exposed blocks is a cliff. Along a run: one rock nub every 2–3 blocks vertically, ±1 lateral variance along the face (re-validated), random 3×3 sub-slot, FACING = out of the wall (same convention as hand placement), waterlogs when placed into water. Registered as `climbamountainvr:cliff_handholds` in `worldgen/ModFeatures` (only the TYPE is code — everything tunable is JSON).
- **Data:** `worldgen/configured_feature/cliff_handholds.json` (empty config) + `worldgen/placed_feature/cliff_handholds.json` (count 6 / in_square / heightmap WORLD_SURFACE_WG / biome) + **Lithostitched modifier** `lithostitched/worldgen_modifier/add_cliff_handholds.json` = `{type: lithostitched:add_features, biomes: "#minecraft:is_mountain", features: ..., step: local_modifications}` (field names confirmed by decompiling Lithostitched 1.7.2's `AddFeaturesModifier` codec: predicate/priority optional, biomes, features, step).
- **Verified headlessly on `runServer` + RCON** (eula + rcon config in `run/`, python RCON client): (1) `/place feature` against an artificial stone cliff placed 14 nubs along the face at the intended ~1-per-3-blocks density; (2) NATURAL generation confirmed — force-generated stony_peaks chunks contained organically-placed nubs. Rock nubs only for now (no outcrop/seam mix), and vanilla-terrain density is sparse — both fine, tuning happens against Tectonic in the pack.
- **Tuning knobs when iterating:** count in the placed feature JSON (no recompile); MIN_CLIFF_RUN + spacing in the feature class (recompile). Consider mixing in slab outcrops as rests + seams for traverses next.
...
- [x] Slip registry + `/climbslip` debug command + tag population (slab outcrop/seam → handhold) — built, loads on server boot ("Loaded 4 climb-slip entries").
- [x] Vivecraft climbing hookup — grip via `vivecraft:climbable` tag bridge, slip via `ClimbTrackerMixin`; boot-verified, NEEDS VR FEEL-TEST (see the hookup note's "WHAT TO TEST IN VR").
- [x] Worldgen scaffolding — cliff-detection feature + Lithostitched biome modifier, verified generating naturally in stony_peaks on a dev server. Tuning vs Tectonic still open.
